...
export default function ReservationActionsMenu({ reservation, refresh, isActive }: ReservationActionsMenuProps) {
  ...
  return (
    <>
      <Menu>
        <MenuButton
          as={IconButton}
          aria-label={t('Reservation actions')}
      ...
      <Box position="absolute" left="-9999px" opacity={0} pointerEvents="none" width="1px" height="1px" overflow="hidden">
        <div ref={viewButtonRef}>
          <ViewReservation reservation={reservation} refresh={refresh} isActive={isActive} />
        </div>
        <div ref={editButtonRef}>
          <EditReservation reservation={reservation} refresh={refresh} isIconButton />
        </div>
        <div ref={deleteButtonRef}>
          <DeleteReservation id={reservation.id} refresh={refresh} isIconButton />
        </div>
      </Box>
    </>
  );
