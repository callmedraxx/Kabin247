...
interface MenuItemActionsMenuProps {
  menuItem: Record<string, any>;
  refresh: () => void;
}

export default function MenuItemActionsMenu({ menuItem, refresh }: MenuItemActionsMenuProps) {
    ...
    <>
      ...
      <Box position="absolute" left="-9999px" opacity={0} pointerEvents="none" width="1px" height="1px" overflow="hidden">
        <div ref={viewButtonRef}>
          <ViewMenuItem menuItem={menuItem} refresh={refresh} />
        </div>
        <div ref={editButtonRef}>
          <EditMenuItem isIconButton editData={menuItem} refresh={refresh} />
        </div>
        <div ref={deleteButtonRef}>
          <DeleteMenuItem isIconButton id={menuItem.id} refresh={refresh} />
        </div>
      </Box>
    </>
  );
